...
public class OpencvOpt {
    public static boolean OpencvOpt(Parameter p,VideoCapture capture,Mat dst,Mat src1,Mat src){
        List<MatOfPoint> list = new ArrayList<MatOfPoint>();
        capture.read(src1);
//        capture.read(src1);


        Rect rect =new Rect(p.getX(),p.getY(),p.getWidth(),p.getLength());
...
//        Imgproc.blur( dst, dst, new Size(3,3) );
                 ...
                 */
                Imgproc.threshold(dst, dst,p.getSsty(), 255, Imgproc.THRESH_BINARY);
...
//       HighGui.destroyWindow("边界框和圆");
        ...
        WebSocket ws=new WebSocket();
            if (list.size()>p.getThred()){
                try {
                   Date t=new Date();
                   SimpleDateFormat df = new SimpleDateFormat("yyyyMMddHHmmss");                              String fileName=df.format(t);
