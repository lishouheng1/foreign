    ...
    <tr>
      <td height="25" align="center" valign="bottom" class="td06"><table width="100%"  border="0" align="center" cellpadding="0" cellspacing="0">
        <tr>
          ...
          <td height="30" valign="middle" background="../../images/bg_03.gif"><div align="left"><font color="#FFFFFF">挂牌清单</font></div></td>
        </tr>
      </table></td>
    </tr>
  </table>
  <form name="form1" method="post" action="/unlist.do">
    <table width="95%" border="0" align="center" cellpadding="0" cellspacing="0" class="table01">
      <tr>
        <td colspan="2" class="td_02"><SPAN class=td_title>查询</SPAN></td>
      </tr>
      <tr>
        <td align="right" width="4%" class="td_02">挂牌人</td>
        <td width="7%" class="td_02"> <select id="reporter" name="reporter" class="input" style="width:99% " >
            <option value="${user.name}" ${reporter==user.name?'selected':''}>--${user.name}--</option>
            <option value="0" ${reporter=='0'?'selected':''}>--所有人--</option>
        </select></td>
    ...
    <table width="95%"  border="0" align="center" cellpadding="0" cellspacing="0">
      <tr>
        <td class="td_page" align="left">
          <div align="right">
            <input name="Submit" type="submit" class="buttonface02" value="挂牌" onClick="openChildenWindowInsert()">
          </div></td>
      </tr>
    </table>
    <table  id="unlist" width="95%" border="0" align="center" cellpadding="0" cellspacing="0" class="table01">
      ...
      <tr>
        <td colspan="7" align="right" class="td07"><a href="/unlist.do?pageNo=1&reporter=${reporter}&area=${area}&line=${line}&equipment=${equipment}"${pageInfo.hasPreviousPage?"":" onclick='return false'"}> <img src="../../images/1.gif" width="4" height="5" align="absmiddle"> 首页　</a><a ${pageInfo.hasPreviousPage?"":" onclick='return false'"} href="/unlist.do?pageNo=${pageInfo.prePage}&reporter=${reporter}&area=${area}&line=${line}&equipment=${equipment}"> <img src="../../images/2.gif" width="3" height="5" align="absmiddle"> 上一页　</a><a href="/unlist.do?pageNo=${pageInfo.pageNum+1}&reporter=${reporter}&area=${area}&line=${line}&equipment=${equipment}" ${pageInfo.hasNextPage?"":" onclick='return false'"}><img src="../../images/2-2.gif" width="3" height="5" align="absmiddle"> 下一页</a> 　<a href="/unlist.do?pageNo=${pageInfo.nextPage}&reporter=${reporter}&area=${area}&line=${line}&equipment=${equipment}"${pageInfo.hasNextPage?"":" onclick='return false'"}> <img src="../../images/3.gif" width="4" height="5" align="absmiddle"> 末页　</a>　共${pageInfo.pages}页当前第${pageInfo.pageNum}页  ${pageInfo.total}条记录</td>
      </tr>
      <tr>
      ...
      <c:forEach items="${pageInfo.list}" var="unlist">
        <tr>
          ...
          <td class="td07" ><a href="#" ${user.name!= unlist.reporter||unlist.status!="未审核"&&unlist.status!="未通过"?"style='display: none' ": "" }id="shanchu" onClick="shanchu(${unlist.id})">删除</a></td>
          <td class="td07"><a href="#" ${user.name != unlist.reporter||unlist.status!="未审核"&&unlist.status!="未通过"?"style='display: none' ": "" } onClick="openChildenWindowUpdate(${unlist.id})">修改</a></div>
          </td>
        </tr>
      </c:forEach>
    </table>
